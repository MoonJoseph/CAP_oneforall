...
function creatingNetwork_ZME(paralist)  %Config is a loaded structure
...
% -------------------------------------------------------------------------
cd(fullfile(pwd,'..'))
fid = fopen(subjlist_file{1}); subjects={}; int=1;
while ~feof(fid);
...
for FCi = 1:numsubj
    ...
    if strcmp(data_type, 'nii')
        unix(sprintf('/bin/cp -af %s %s', [imagefilter, '*.nii*'], temp_dir));                          %core
        if exist('unused', 'dir')
        unix(sprintf('/bin/cp -af %s %s', fullfile('unused', [imagefilter, '*.nii*']), temp_dir));       %core
        end
    else
        unix(sprintf('/bin/cp -af %s %s', [imagefilter, '*.img*'], temp_dir));                           %core
        unix(sprintf('/bin/cp -af %s %s', [imagefilter, '*.hdr*'], temp_dir));                            %core
        if exist('unused', 'dir')
          unix(sprintf('/bin/cp -af %s %s', fullfile('unused', [imagefilter, '*.img*']), temp_dir));     %core
          unix(sprintf('/bin/cp -af %s %s', fullfile('unused', [imagefilter, '*.hdr*']), temp_dir));     %core
        end 
    end
    cd(temp_dir);
    unix('gunzip -fq *');
    ...
    if bandpass_on == 1
        ...
        bandpass_final_SPM(2, fl, fh, temp_dir, imagefilter, data_type);                    %core
        disp('Done');
    ...
    [all_roi_ts, roi_names] = extract_ROI_timeseries_eigen1(roi_file, temp_dir, 1, ...
                                      0, newimagefilter, data_type);                         %core
    ...
    [wm_csf_ts, ~] = extract_ROI_timeseries_eigen1(wm_csf_roi_file, temp_dir, 1, ...         %core
    ...
    all_roi_ts = all_roi_ts(NUMTRUNC(1)+1:NFRAMES-NUMTRUNC(2), :);                           %core
    %global_ts = org_global_ts(NUMTRUNC(1)+1:end-NUMTRUNC(2));
    
    %wm_csf_ts = wm_csf_ts(NUMTRUNC(1)+1:end-NUMTRUNC(2), :);
    wm_csf_ts = wm_csf_ts(NUMTRUNC(1)+1:NFRAMES-NUMTRUNC(2), :);                             %core
   ...
   for roicnt = 1:numroi                              
    ...
    rts = rts - mean(rts)*ones(size(rts, 1), 1);                                     %core
   
    %-bandpass filtering the movement parameters
    if bandpass_on == 1
        mvmnt = bandpass_final_SPM_ts(TR_val, fl, fh, mvmnt);                         %core
    end
    %mvmnt = mvmnt(NUMTRUNC(1)+1:NUMTRUNC(1)+NumVolsKept, :);
    mvmnt = mvmnt(NUMTRUNC(1)+1:NFRAMES-NUMTRUNC(2), :);                               %core  
	...
	if(strcmpi(CONNECTIVITY_MEASURE,'pearson'))
		...
		W = ComputePearsonCorrelations(all_roi_ts);                                       %core
    elseif (strcmpi(CONNECTIVITY_MEASURE,'wavelet'))
		W = ComputeWaveletCorrelations(all_roi_ts);                                      %core
    end
	cd(outputdir);
